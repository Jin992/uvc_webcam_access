...
:current_cnt_(0), device_name_("/dev/video0"), height_(480), width_(640), fps_(30 * 100), bitrate_(0),
io(IO_METHOD_USERPTR), fmt_(V4L2_PIX_FMT_RGB24), field(V4L2_FIELD_INTERLACED), fd(-1)
{}

VideoCapture::VideoCapture(std::string  dev, uint16_t width, uint16_t height, uint16_t fps, uint32_t bitrate)
: current_cnt_(0), device_name_(dev), height_(height), width_(width), bitrate_(bitrate), fps_(fps * 100)
{}

void VideoCapture::set_params(std::string &dev, uint16_t width, uint16_t height, uint16_t fps, uint32_t bitrate) {
    device_name_ = dev;
    width_ = width;
    height_ = height;
    bitrate_ = bitrate;
...
void VideoCapture::_measure_bitrate(int used_bytes) {
...
int VideoCapture::_process_image(int n, int used_bytes) {
    // memorize last buffer number
    buffer_last = n;
    // write frame to stdout
    write(1, buffers[n].start, used_bytes);
...
int VideoCapture::_read_frame() {
    ...
    return n >= 0 ? _process_image(n, used) : 0;
}

/* set timeout to wait frame */
int VideoCapture::_shot(u_int32_t tout) {
    fd_set fds;
    struct timeval tv;
    int r;
    ...
    r = select(fd + 1, &fds, NULL, NULL, tout == -1 ? NULL : &tv);
    // error
    if(r <= 0) {
        // not error
        if(r < 0 && errno == EINTR) {
...
DRIVER=uvcvideo
PHYSDEVBUS=usb
PHYSDEVDRIVER=uvcvideo
DEVICE=/proc/bus/usb/002/011
PRODUCT=46d/82d/11
TYPE=239/2/1
INTERFACE=14/1/0
MODALIAS=usb:v046Dp082Dd0011dcEFdsc02dp01ic0Eisc01ip00
     */
    if(f) {
        char buf[MAX_STR];
...
void VideoCapture::capture_video_stream(std::mutex &mutex) {
    int r = _start_capturing();
    if(!r) {
        err(1, "start_capturing");
    }
    int bitrate = 100000;
    struct timespec ts, t, tf;
    ...
    while(1) {
        ...
        mutex.lock();
        if (bitrate_switch_) {
            video_H264_set_bitrate(/*bitrate*/bitrate_);
            bitrate_switch_ = false;
            std::cerr << "Bitrate changed to " << bitrate <<std::endl;
        }
        mutex.unlock();
        i++;
        r = _shot(1000000);
...
void VideoCapture::set_new_bitrate(uint32_t bitrate) {
    bitrate_ = bitrate;
    bitrate_switch_ = true;
}
